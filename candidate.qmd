...
motif.path %>%
  ...
  transmute(
    'CRS ID' = motif,
    'CRS anchoring orthologous gene ID' = term,
    'CRS anchoring orthologous gene name' = ortholog,
    'Relative position' = side,
    'CRS category' = category,
    'CRS length' = alignment.len,
    'No. sequences' = no.seq,
    'No. species' = no.species,
    'Average sequence identity, %' = `Avgerage SI %`,
    'GC content %' = GC * 100,
    "No. base-pairs" = no.bps,
    'Proportion basepaired alignment positions, %' = `Paired positions %`,
    'FDR' = RNAphylo.fdr,
    'Transcriptionally active genomes' = expressed.in.genomes,
    'RNAphylo score (CMfinder)' = `RNAphylo`,
    'Hmmpair score (CMfinder)' = `hmmpair`,
    'Expected covarying basepairs (R-scape)' = `expected.covary`,
    'Alignment power % (R-Scape)' = `Alignment power %`,
    'Observed covarying basepairs (R-scape)' = `observed.covary`,
    'Covarying basepairs % (R-Scape)' = `Covarying bps %`
  ) |>
...
crossing(motif = motif$motif, genome = genome.cl$genome) |>
  ...
  left_join(genome.cl, 'genome') -> expr.dat
